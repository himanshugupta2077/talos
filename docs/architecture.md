...
| `talos.intruder` | High-volume mutation engine (Phase 1–3): template + generators (wordlist/numbers/static/uuid/csv/json/example_values/pool) + single/sniper/pitchfork/zip/cluster_bomb + fixed RPS; storage modes; host concurrency caps; session clone; grep extract → `intruder_pools`; param-intel `from-params`; time-sliced `intruder_session` jobs; metrics table; no findings bridge | Adaptive timing, AI suggest, UI (later) |
