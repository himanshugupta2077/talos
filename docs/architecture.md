...
| `talos.intruder` | High-volume mutation engine (Phase 1+2): template + generators + single/sniper/pitchfork/zip/cluster_bomb + fixed RPS; storage modes (metrics/sample/all); host concurrency caps; session clone; time-sliced `intruder_session` jobs; metrics table; no findings bridge | Grep pools, adaptive timing, UI (later) |
