...
| **Author** | _(TBD)_ |
| **Date** | 2026-07-29 |
| **Status** | Draft (rev 5b — Workflow Engine, ToolSpec/Policy split, immutable suggestions, capabilities; **AI client-data redaction out of scope** for authorized BB/pentest use). **Phase A implemented** (2026-07-29): sessions/pin/budgets/audit + TTP + PolicyValidator/Executor + READ/context tools + schema v49. **Phase B implemented** (2026-07-29): app notes v50 + immutable suggestions/plans/obs/PTT v51 + heuristic planner + suggest/approve/deny offline loop. **Phase C implemented** (2026-07-29): stdio MCP + LLM providers (none/ollama/openai-compatible/anthropic) + operator config; **no** redaction module. **Phase D implemented** (2026-07-29): send.once + replay.flow + live scope/annotation matrix + IV/passive/unauth/BAC/intruder enqueue tools (PR7+PR8a+PR8b). **Phase E core CLI implemented** (2026-07-29): minimal markdown KB (`~/.talos/ai/kb/*.md`) + draft findings promote (schema v52) + session export; **Control Panel AI page deferred**. |
