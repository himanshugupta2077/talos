...
│            / cors / sqli / xss / path-traversal / ssrf / open-redirect / smuggle
...
| `talos.scheduler.scheduler.ReplayScheduler` | Daemon thread: consume pending jobs from scheduler_jobs; annotation pre-check (logout/dangerous); optional IST testing-window hold (no HTTP / no IV-unauth auto-enqueue outside windows); per-cycle config reload; configurable jitter; mark job done/failed/skipped (`endpoint_excluded` / `endpoint_not_qualified` → skipped); trigger `create_finding_from_verdict` after BAC and auth outcomes | Direct execution (delegates to replay/auth engines), CLI parsing |
...
| `xss_results` | XSS / HTMLI: one verdict row per unique payload replay flow (v64) |
...
| XSS / HTML injection | `xss_attack` (one unique replay flow per entry point × payload) |
...
| v63 | Host-header injection: `host_header_results` |
| v64 | XSS / HTML injection: `xss_results` |
...
- [x] Authentication Provider architecture — per-role provider selection (AUTO | MANUAL) stored in `role_auth_provider`; MANUAL sessions store arbitrary headers/cookies/expiry in `manual_session_config` and are applied to `role_auth_state` without replaying any flows; AUTO provider retains the existing extractor-based refresh path; `session_health.should_refresh`, `refresh_auth_state`, and `ensure_healthy` are provider-aware; scheduler detects MANUAL session expiry, pauses itself (`WAITING_FOR_SESSION`), and marks pending jobs `paused` instead of failing them; `talos auth-config set-provider`, `set-session`, `show-provider`, `clear-session`, `reset-health`; `talos scheduler pause|resume` (`talos.projects.auth_provider`, `talos.projects.session_health`, `talos.scheduler`)
  ...
  - **Input Validation auth pre-check:** `talos input-validation run` (and all phase shorthand commands) call `verify_auth_for_iv_scan()` before scheduling any jobs. This collects all role_ids from flows for the scoped endpoints. When no HTTP artifacts are configured but a credentialed `talos proxy auth` (platform NTLM) profile covers every scoped host, the scan is ready — NTLM is the session and role cookie/extractor checks are skipped (`talos.projects.auth_mechanism`). Otherwise each role must have: auth artifact names configured, provider set, session values / login flows present, validation method configured, and validation passes. If any role fails the check, the scan does not start.
  - **Platform NTLM as auth mechanism:** IIS Persistent-Auth does not put an `Authorization` header on captured requests after the handshake. Unauth and `talos auth test` send with `create_async_client(..., platform_auth=False)` so the origin handshake is not re-applied. `ensure_healthy()` returns True immediately for NTLM-only projects. `talos auth show` lists platform NTLM profiles. Header-mutation unauth techniques still require HTTP artifact names.
