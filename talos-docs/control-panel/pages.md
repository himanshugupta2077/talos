...
| `/testing/xss` | XSS / HTML Injection (Active) — HTML/JS, HTMLI, encoded, bypass probes on a chosen flow |
| `/testing/smuggle` | HTTP Request Smuggling (Active) — raw CL/TE probes on a chosen flow |
...
| **Nav** | Sidebar group **Testing** → **Scheduler** + **Attack Module** (`/testing`). Available Active modules (Unauth, BAC, Auth-Session, Input Validation, CORS, SQLi, XSS, Smuggle, Intruder) nest under Attack Module. Passive modules stay hub-only. |
...
| **Run** | Flow UUID list, Select DB (unknown / Microsoft SQL Server), optional parameter, family/technique picker, high-priority checkbox (default on), enqueue → `attack sqli run --flow … [--db mssql] [--param name] [--high-priority]` |
...
**Files:** `pages/attack/modules/XssModule.tsx` + `pages/attack/xss/*`

Tabbed workspace for `talos attack xss …`. Operator must pick a flow (paste UUIDs or use the Flows table attack bar).

| Tab | Role |
|-----|------|
| **Overview** | XSS / HTMLI / SECURE / UNKNOWN KPIs, job pressure, recent confirmed issues |
| **Run** | Flow UUID list, optional parameter, family/technique picker, high-priority checkbox (default on), enqueue → `attack xss run --flow … [--param name] [--family html_tag] [--high-priority]` |
| **Results** | Filterable table (verdict, family, technique, host); row opens the unique replay flow |

Finding rule (core): `TalosXss` canary reflected with an unencoded JS sink → XSS, or unencoded HTML markup → HTMLI. Both create PRIMARY (`XSS:<endpoint_id>`), later probes LINKED. Encoded echo is not a finding.

### HTTP Request Smuggling (`/testing/smuggle`)

**Files:** `pages/attack/modules/SmuggleModule.tsx` + `pages/attack/smuggle/*`

Tabbed workspace for `talos attack smuggle …`. Operator must pick a flow (paste UUIDs or use the Flows table attack bar).

| Tab | Role |
|-----|------|
| **Overview** | SMUGGLE / SECURE / UNKNOWN KPIs, job pressure, recent confirmed desyncs |
| **Run** | Flow UUID list, technique picker, enqueue → `attack smuggle run --flow …` |
| **Results** | Filterable table (verdict, technique, NTLM, host); row opens the unique replay flow |

Finding rule (core): confirmed desync (poisoned follow-up / canary / extra response) → PRIMARY (`SMUGGLE:<origin>`), later techniques LINKED. Timeout-only is not a finding. NTLM hosts handshake on the same keep-alive connection. Requests appear in the Talos Burp extension.
