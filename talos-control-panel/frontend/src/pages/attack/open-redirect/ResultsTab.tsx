...
export default function ResultsTab({
  ...
  const [results, setResults] = useState<OpenRedirectResultRow[]>([]);
  ...
  const load = useCallback(() => {
    setLoading(true);
    api
      .get<{ results: OpenRedirectResultRow[] }>("/api/attack/open-redirect/results", {
  ...
  const columns: Column<OpenRedirectResultRow>[] = [
    {
      key: "captured_at",
      header: "Time",
    ...
    },
    {
      ...
      className: "mono text-xs",
      render: (r) => (
        <span className="break-all" title={r.redirect_url || undefined}>
          {(r.redirect_url || "—").slice(0, 48)}
        </span>
      ),
      defaultWidth: 180,
    },
    ...
    },
    {
      ...
      sortable: false,
      render: (r) => (
        <span className="break-all" title={r.evidence}>
          {(r.evidence || r.risk_hint || "—").slice(0, 48)}
        </span>
      ),
      defaultWidth: 180,
    },
    ...
    <div>
      ...
      <Section title={loading ? "Results (loading…)" : `Results (${results.length})`}>
        <DataTable
          ...
          emptyLabel="No open-redirect probe results yet. Pick a flow and enqueue a scan."
        />
      </Section>
    </div>
  );
