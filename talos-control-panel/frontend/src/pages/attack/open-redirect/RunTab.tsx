...
import OpenRedirectDisclaimer from "./components/OpenRedirectDisclaimer";

interface OpenRedirectPoint {
  flow_id?: string;
  location: string;
  name: string;
  original?: string;
...
export default function RunTab({
  ...
  projectId: string;
  overview: OpenRedirectOverview | null;
  ...
  const [points, setPoints] = useState<OpenRedirectPoint[]>([]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    api
      .get<{ points?: OpenRedirectPoint[] }>("/api/attack/open-redirect/points", {
  ...
  const run = useAction("Run open-redirect attack", () =>
    api.post(
  ...
  const cliPreview = useMemo(() => {
    const parts = ["talos attack open-redirect run"];
    ...
    <div className="space-y-4">
      ...
      <Section title="Flows">
        <p className="text-xs text-base-content/60 mb-2">
          ...
          and run Open Redirect from the attack bar.
        </p>
        <textarea
          ...
          <div className="flex flex-wrap items-center gap-2">
            {estimate > 50 ? (
              <ConfirmButton
                ...
                confirmText={`Enqueue open-redirect jobs for ${flowIds.length} flow(s)?`}
                onConfirm={doRun}
              >
                {run.running ? (
                  <span className="loading loading-spinner loading-xs" />
                ) : (
                  "Enqueue open-redirect scan"
                )}
              </ConfirmButton>
            ) : (
              <button
