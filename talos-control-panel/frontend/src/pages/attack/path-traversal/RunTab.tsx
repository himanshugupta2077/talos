...
export default function RunTab({
  ...
  techniques: PathTraversalTechnique[];
  onRefresh: () => void;
}) {
  ...
  const [points, setPoints] = useState<PathTraversalPoint[]>([]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    api
      .get<{ points?: PathTraversalPoint[] }>("/api/attack/path-traversal/points", {
  ...
  const run = useAction("Run path-traversal attack", () =>
    api.post(
  ...
  const cliPreview = useMemo(() => {
    const parts = ["talos attack path-traversal run"];
    ...
    <div className="space-y-4">
      ...
      <Section title="Flows">
        <p className="text-xs text-base-content/60 mb-2">
          ...
          and run Path Traversal from the attack bar.
        </p>
        <textarea
          ...
          <div className="flex flex-wrap items-center gap-2">
            {estimate > 50 ? (
              <ConfirmButton
                ...
                confirmText={`Enqueue path-traversal jobs for ${flowIds.length} flow(s)?`}
                onConfirm={doRun}
              >
                {run.running ? (
                  <span className="loading loading-spinner loading-xs" />
                ) : (
                  "Enqueue path-traversal scan"
                )}
              </ConfirmButton>
            ) : (
              <button
