...
export default function ResultsTab({
  ...
  const [results, setResults] = useState<PathTraversalResultRow[]>([]);
  ...
  const load = useCallback(() => {
    setLoading(true);
    api
      .get<{ results: PathTraversalResultRow[] }>("/api/attack/path-traversal/results", {
  ...
  const columns: Column<PathTraversalResultRow>[] = [
    {
      key: "captured_at",
      header: "Time",
    ...
    <div>
      ...
      <Section title={loading ? "Results (loading…)" : `Results (${results.length})`}>
        <DataTable
          ...
          emptyLabel="No path-traversal probe results yet. Pick a flow and enqueue a scan."
        />
      </Section>
    </div>
  );
