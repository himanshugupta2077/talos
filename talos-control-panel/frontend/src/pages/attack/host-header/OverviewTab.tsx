...
import type { HostHeaderOverview, HostHeaderTab } from "./shared";
...
export default function OverviewTab({
  ...
  onRefresh: () => void;
  onGoTab: (t: HostHeaderTab) => void;
}) {
  const counts = overview?.counts || {};
  const issues = counts.HOST_HEADER ?? 0;
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
        <div className="panel p-3">
          <div className="text-xs text-base-content/50">Confirmed host-header</div>
      ...
      {empty.no_results && (
        <div className="alert text-xs mb-4">
          No host-header probes yet. Open a captured flow and run Host Header
          Injection, or paste a flow UUID on the Run tab.
        </div>
      )}

      <Section title="Recent issues">
        {(overview?.recent_issues || []).length === 0 ? (
          <p className="text-sm text-base-content/50">No HOST_HEADER results yet.</p>
        ) : (
            ...
            {(overview?.recent_issues || []).map((row) => (
              <li key={row.replay_flow_id} className="panel p-3 text-xs">
                ...
                <div className="mono text-base-content/60 break-all">
                  {row.param_name} {row.risk_hint ? `· ${row.risk_hint}` : ""}{" "}
