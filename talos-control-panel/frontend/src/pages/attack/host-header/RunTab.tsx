...
import HostHeaderDisclaimer from "./components/HostHeaderDisclaimer";

interface HostHeaderPoint {
  flow_id?: string;
  location: string;
  name: string;
  original?: string;
...
export default function RunTab({
  ...
  projectId: string;
  overview: HostHeaderOverview | null;
  ...
  const [points, setPoints] = useState<HostHeaderPoint[]>([]);

  const flowIds = useMemo(() => parseFlowIds(flowText), [flowText]);
  const visibleTechniques = techniques.filter((t) => !family || t.family === family);
  ...
  const techCount = technique
    ...
    : family
      ...
      : overview?.total_techniques || techniques.length || 0;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    api
      .get<{ points?: HostHeaderPoint[] }>("/api/attack/host-header/points", {
  ...
  const run = useAction("Run host-header attack", () =>
    api.post(
  ...
  const cliPreview = useMemo(() => {
    const parts = ["talos attack host-header run"];
    for (const id of flowIds) parts.push(`--flow ${id}`);
    if (paramName.trim()) parts.push(`--header ${paramName.trim()}`);
    ...
    return parts.join(" ");
  }, [flowIds, paramName, technique, family, highPriority]);
  ...
  return (
    <div className="space-y-4">
      <HostHeaderDisclaimer />

      {jobsInFlight && (
        <div className="alert alert-warning text-xs py-2">
          <span>
            Host-header jobs already in flight ({overview?.jobs_running ?? 0}{" "}
      ...
      <Section title="Flows">
        <p className="text-xs text-base-content/60 mb-2">
          ...
          and run Host Header Injection from the attack bar.
        </p>
        <textarea
        ...
        <p className="text-xs text-base-content/50 mt-1">
          ...
          {paramName.trim()
            ? ` on ${paramName.trim()}`
            : " (times Host and override headers)"}
        </p>
      </Section>

      <Section title="Header">
        <p className="text-xs text-base-content/60 mb-2">
          Optional. Leave as all host-related headers, or scan one of Host,
          X-Forwarded-Host, Forwarded, and related override headers.
        </p>
        <div className="flex flex-wrap gap-2 items-center">
          <select
            ...
            disabled={run.running}
          >
            <option value="">All host headers</option>
            {paramName &&
              !uniqueParams.some(
                (p) =>
                  p.name === paramName ||
          ...
          <input
            ...
            placeholder="or type Host / header:Name"
            value={paramName}
        ...
        {flowIds.length > 0 && uniqueParams.length === 0 && (
          <p className="text-xs text-base-content/40 mt-1">
            No headers loaded yet for the pasted flow(s).
          </p>
        )}
      </Section>
      ...
      <Section title="Enqueue">
        <div className="panel p-4 space-y-3">
          <p className="text-sm">
            ~{estimate}+ job{estimate === 1 ? "" : "s"} (one per header ×
          ...
          <div className="flex flex-wrap items-center gap-2">
            {estimate > 50 ? (
              <ConfirmButton
                ...
                confirmText={`Enqueue host-header jobs for ${flowIds.length} flow(s)?`}
                onConfirm={doRun}
              >
                {run.running ? (
                  <span className="loading loading-spinner loading-xs" />
                ) : (
                  "Enqueue host-header scan"
                )}
              </ConfirmButton>
            ) : (
              <button
