...
import XssDisclaimer from "./components/XssDisclaimer";
...
export default function RunTab({
  ...
  const [points, setPoints] = useState<XssPoint[]>([]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    api
      .get<{ points?: XssPoint[] }>("/api/attack/xss/points", {
  ...
  const run = useAction("Run XSS attack", () =>
    api.post(
      "/api/attack/xss/run",
      {
        flows: flowIds,
  ...
  const cliPreview = useMemo(() => {
    const parts = ["talos attack xss run"];
    ...
    <div className="space-y-4">
      ...
      {jobsInFlight && (
        <div className="alert alert-warning text-xs py-2">
          <span>
            XSS jobs already in flight ({overview?.jobs_running ?? 0}{" "}
      ...
      <Section title="Flows">
        <p className="text-xs text-base-content/60 mb-2">
          ...
          and run XSS from the attack bar.
        </p>
        <textarea
          ...
          {visibleTechniques.map((t) => (
            ...
            >
              ...
              <div className="text-base-content/40 mt-1">
                {t.family}
                {t.risk_class && t.risk_class !== "xss" ? ` · ${t.risk_class}` : ""}
          ...
          <div className="flex flex-wrap items-center gap-2">
            {estimate > 50 ? (
              <ConfirmButton
                ...
                confirmText={`Enqueue XSS jobs for ${flowIds.length} flow(s)?`}
                onConfirm={doRun}
              >
                {run.running ? (
