...
export default function ResultsTab({
  ...
  const [results, setResults] = useState<XssResultRow[]>([]);
  ...
  const load = useCallback(() => {
    setLoading(true);
    api
      .get<{ results: XssResultRow[] }>("/api/attack/xss/results", {
  ...
  const columns: Column<XssResultRow>[] = [
    {
      key: "captured_at",
      header: "Time",
    ...
    <div>
      ...
      <Section title={loading ? "Results (loading…)" : `Results (${results.length})`}>
        <DataTable
          ...
          emptyLabel="No XSS probe results yet. Pick a flow and enqueue a scan."
        />
      </Section>
    </div>
  );
