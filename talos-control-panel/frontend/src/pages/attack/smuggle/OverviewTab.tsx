...
import type { SmuggleOverview, SmuggleTab } from "./shared";
...
export default function OverviewTab({
  ...
  onRefresh: () => void;
  onGoTab: (t: SmuggleTab) => void;
}) {
  const counts = overview?.counts || {};
  const issues = counts.SMUGGLE ?? 0;
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
        <div className="panel p-3">
          <div className="text-xs text-base-content/50">Confirmed desync</div>
      ...
      {empty.no_results && (
        <div className="alert text-xs mb-4">
          No smuggle results yet. Paste a captured flow UUID on the Run tab
          (or select rows on Flows) and enqueue.
        </div>
      )}
            ...
            {(overview?.recent_issues || []).map((row) => (
              <li key={row.replay_flow_id} className="panel p-3 text-xs">
                ...
                <div className="mono text-base-content/60 break-all">
                  {row.desync_signal || "desync"} · baseline {row.baseline_status ?? "—"} →
                  follow-up {row.followup_status ?? "—"}
                  {row.ntlm_used ? " · NTLM" : ""}
                </div>
              </li>
            ))}
