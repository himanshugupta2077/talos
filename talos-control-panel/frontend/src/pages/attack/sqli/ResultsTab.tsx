...
export default function ResultsTab({
  ...
  const [results, setResults] = useState<SqliResultRow[]>([]);
  ...
  const load = useCallback(() => {
    setLoading(true);
    api
      .get<{ results: SqliResultRow[] }>("/api/attack/sqli/results", {
  ...
  const columns: Column<SqliResultRow>[] = [
    {
      key: "captured_at",
      header: "Time",
    ...
    <div>
      ...
      <Section title={loading ? "Results (loading…)" : `Results (${results.length})`}>
        <DataTable
          ...
          emptyLabel="No SQLi probe results yet. Pick a flow and enqueue a scan."
        />
      </Section>
    </div>
  );
