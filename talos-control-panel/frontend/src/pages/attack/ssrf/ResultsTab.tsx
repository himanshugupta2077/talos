...
export default function ResultsTab({
  ...
  const [results, setResults] = useState<SsrfResultRow[]>([]);
  ...
  const load = useCallback(() => {
    setLoading(true);
    api
      .get<{ results: SsrfResultRow[] }>("/api/attack/ssrf/results", {
  ...
  const columns: Column<SsrfResultRow>[] = [
    {
      key: "captured_at",
      header: "Time",
    ...
    <div>
      ...
      <Section title={loading ? "Results (loading…)" : `Results (${results.length})`}>
        <DataTable
          ...
          emptyLabel="No ssrf probe results yet. Pick a flow and enqueue a scan."
        />
      </Section>
    </div>
  );
