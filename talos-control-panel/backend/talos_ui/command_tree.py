...
COMMAND_TREE: list[dict] = [
    ...
    },
    {
        ...
        "commands": [
            cmd("attack.sqli.techniques", ["attack", "sqli", "techniques"], "List SQLi payload techniques", [
                arg(
                    "db",
                    flag="--db",
                    kind="select",
                    options=["unknown", "mssql"],
                    help="Filter techniques for unknown (multi-vendor) or mssql",
                ),
            ]),
            cmd("attack.sqli.run", ["attack", "sqli", "run"], "Enqueue SQLi probes for selected flows", [
                ...
                arg(
                    ...
                    options=["unknown", "mssql"],
                    default="unknown",
                    help="Target DBMS: unknown (multi-vendor + encodings) or mssql (Microsoft SQL Server)",
                ),
                arg(
                    "param",
                    flag="--param",
                    help="Restrict to one entry point (query key, JSON path, form field). Repeatable.",
                ),
                arg(
                    "technique",
                    flag="--technique",
                ...
                arg(
                    ...
                    flag="--family",
                    kind="select",
                    options=["error", "union", "boolean", "time"],
