"""CLI tests for talos attack open-redirect (enqueue unique jobs, no HTTP)."""
...
from talos.open_redirect.cli import (
    build_open_redirect_parser,
    cmd_run,
    cmd_techniques,
)
...
from talos.scheduler.job import OPEN_REDIRECT_ATTACK, PRIORITY_HIGH

PROJECT_ID = "proj-or-cli"
EP = "ep-or-cli"
FLOW = "flow-or-cli"
...
def db_path(tmp_path: Path) -> Path:
    ...
    with sqlite3.connect(str(path)) as conn:
        ...
        conn.execute(
            ...
            VALUES (?, ?, 'GET', 'https://app.example.com', '/out', '/out',
                    'text/html', 0, '[]', ?, ?)
            """,
        ...
        conn.execute(
            ...
            VALUES (?, ?, ?, 'GET',
                    'https://app.example.com/out?next=/home',
                    'app.example.com', '/out', 'next=/home', ?, ?, 200, ?,
                    ?, ?, '[]', 'proxy_capture')
            """,
            (
                FLOW,
...
def test_run_parser_accepts_param() -> None:
    ...
    args = parser.parse_args(
        ["open-redirect", "run", "--flow", FLOW, "--param", "next"]
    )
    assert args.params == ["next"]
...
def test_techniques_json() -> None:
    ...
    assert "abs_https" in names
    assert "pr_slash" in names
    assert "js_alert" in names
...
def test_run_enqueues_high_priority_jobs(db_path: Path) -> None:
    ...
    assert nxt.job_type == OPEN_REDIRECT_ATTACK
    meta = json.loads(nxt.meta)
    assert CANARY_HOST in meta["payload_sent"]
