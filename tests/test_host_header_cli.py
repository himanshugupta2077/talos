"""CLI tests for talos attack host-header (enqueue unique jobs, no HTTP)."""
...
from talos.host_header.cli import (
    build_host_header_parser,
    cmd_run,
    cmd_techniques,
)
...
def db_path(tmp_path: Path) -> Path:
    ...
    with sqlite3.connect(str(path)) as conn:
        ...
        conn.execute(
            ...
            VALUES (?, ?, 'GET', 'https://app.example.com', '/reset', '/reset',
                    'text/html', 0, '[]', ?, ?)
            """,
        ...
        conn.execute(
            ...
            (
                ...
                PROJECT_ID,
                NOW,
                json.dumps({"Host": "app.example.com"}),
                b"",
                EP,
                role,
                module,
...
def test_run_parser_accepts_header() -> None:
    ...
    assert args.params == ["Host", "X-Forwarded-Host"]
...
def test_run_enqueues_high_priority_jobs(db_path: Path) -> None:
    ...
    assert nxt.job_type == HOST_HEADER_ATTACK
