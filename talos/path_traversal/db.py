"""
Module: talos.path_traversal.db

Purpose:
    Persist and query path_traversal_results — one row per unique replay flow.

Dependencies: sqlite3, pathlib, talos.projects.db
Data flow: engine insert → CLI / Control Panel list/show
Side effects: writes path_traversal_results; migrate_project_db on entry.
"""
...
def _now_iso() -> str:
    """Purpose: UTC timestamp for path_traversal_results.created_at."""
...
def insert_path_traversal_result(db_path: Path, row: dict) -> None:
    """
    Purpose:
        Store one path-traversal probe result keyed by the unique replay flow.
    Side effects: INSERT into path_traversal_results.
    """
    migrate_project_db(db_path)
    with _connect_rw(db_path) as conn:
        conn.execute(
            """
            INSERT INTO path_traversal_results (
                ...
                os_hint, evidence, verdict, risk_hint,
...
def list_path_traversal_results(
    db_path: Path,
    *,
    ...
    """
    Purpose:
        List path-traversal probe results joined to the unique replay flow.
    Output:
    ...
    with _connect_rw(db_path) as conn:
        rows = conn.execute(
            ...
            SELECT pr.*,
                   ...
                   f.captured_at
            FROM path_traversal_results pr
...
def get_path_traversal_result(db_path: Path, replay_flow_id: str) -> Optional[dict]:
    """
    Purpose:
        Fetch one path_traversal_results row by unique replay flow UUID.
    Output:
        Dict or None.
    """
    migrate_project_db(db_path)
...
def count_path_traversal_verdicts(db_path: Path) -> dict[str, int]:
    """
    Purpose:
    ...
    with _connect_rw(db_path) as conn:
        rows = conn.execute(
            "SELECT verdict, COUNT(*) AS n FROM path_traversal_results GROUP BY verdict"
        ).fetchall()
