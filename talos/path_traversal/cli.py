"""
Module: talos.path_traversal.cli

Purpose:
    Command-line interface for path traversal / LFI testing.
    Entry point: talos attack path-traversal <subcommand>

    Subcommands:
        techniques  — catalogue of LFI / traversal payloads
        run         — enqueue one path_traversal_attack job per
...
Dependencies: argparse, talos.cli_output, talos.path_traversal.*, talos.scheduler
Data flow: attack_cli → run_path_traversal_cli → enqueue / engine
...
from talos.path_traversal.candidates import (
    PathTraversalCandidate,
    normalize_flow_ids,
    select_path_traversal_candidates_for_flows,
)
from talos.path_traversal.db import (
    count_path_traversal_verdicts,
    get_path_traversal_result,
    list_path_traversal_results,
)
from talos.path_traversal.inject import match_injection_points, normalize_param_names
from talos.path_traversal.models import FAMILIES, InjectionPoint
from talos.path_traversal.payloads import (
    TECHNIQUE_CATALOG,
...
def _select(manager: ProjectManager, args: argparse.Namespace) -> list[PathTraversalCandidate]:
    ...
    if not project.scope:
        cli_precondition_error(
            ...
            "'talos project scope' before path-traversal testing."
        )
    candidates, missing = select_path_traversal_candidates_for_flows(
        project.db_path,
...
def _has_pending_duplicate(
    ...
    """Purpose: Skip identical pending/running path_traversal_attack jobs."""
    with sqlite3.connect(str(db_path)) as conn:
        row = conn.execute(
            ...
            LIMIT 1
            """,
            (PATH_TRAVERSAL_ATTACK, flow_id, param_name, technique),
        ).fetchone()
    return row is not None


def cmd_techniques(manager: ProjectManager, args: argparse.Namespace) -> None:
    """Purpose: Print the path-traversal technique catalogue. manager unused."""
    del manager
    ...
    if family:
        if family not in FAMILIES:
            cli_usage_error(
                f"unknown path-traversal family {family!r}. "
    ...
    if wants_json(args):
        cli_json(catalog)
        return
    print(f"{'TECHNIQUE':<28} {'FAMILY':<10} {'OS':<8} DESCRIPTION")
    for item in catalog:
        print(
            f"{str(item['name']):<28} {str(item['family']):<10} "
            f"{str(item.get('os') or ''):<8} {item['description']}"
        )
...
def cmd_run(manager: ProjectManager, args: argparse.Namespace) -> None:
    """
    Purpose:
        Enqueue path_traversal_attack jobs (or execute immediately with --right-now).
    ...
    if not candidates:
        cli_error(
            "None of the specified flows are usable path-traversal targets "
    ...
    if param_filters:
        narrowed: list[PathTraversalCandidate] = []
        for cand in usable:
    ...
    planned: list[tuple[PathTraversalCandidate, InjectionPoint, object]] = []
    for cand in usable:
    ...
    for cand, point, payload in planned:
        ...
        sched_db.enqueue_job(
            db_path=db_path,
            job_id=job_id,
            job_type=PATH_TRAVERSAL_ATTACK,
    ...
    print("\nPath traversal attack generation complete.")
    ...
    print(
        ...
        "extension under Path Traversal). "
        "Run 'talos scheduler status' to monitor execution. "
        "Use 'talos attack path-traversal results list' and "
...
def _run_right_now(
    ...
    *,
    ...
    Side effects: outbound HTTP; flows + path_traversal_results + findings.
    """
    ...
    print(f"\nPath traversal --right-now complete. Probes: {len(outcomes)}")
    for row in outcomes:
...
def cmd_results_list(manager: ProjectManager, args: argparse.Namespace) -> None:
    """Purpose: List stored path-traversal probe results (unique replay flows)."""
    project = _require_active(manager)
    ...
    if not rows:
        cli_info(
            "No path-traversal results yet. "
            "Run 'talos attack path-traversal run --flow <uuid>'."
        )
        return
    print(
        f"{'VERDICT':<16} {'FAMILY':<10} {'TECH':<24} {'PARAM':<18} "
        f"{'OS':<8} {'METHOD':<7} {'PATH':<24} REPLAY"
    )
    for row in rows:
        print(
...
def cmd_results_show(manager: ProjectManager, args: argparse.Namespace) -> None:
    """Purpose: Show one path-traversal result by replay flow UUID."""
    project = _require_active(manager)
    replay_id = args.replay_flow_id
    row = get_path_traversal_result(project.db_path, replay_id)
    if row is None:
        cli_error(f"Path-traversal result not found: {replay_id}")
    if wants_json(args):
        cli_json(row)
        return
    fields = {
    ...
    cli_success("Path-traversal probe result", fields)


def cmd_status(manager: ProjectManager, args: argparse.Namespace) -> None:
    """Purpose: Verdict histogram + path_traversal_attack job pressure."""
    project = _require_active(manager)
    counts = count_path_traversal_verdicts(project.db_path)
...
def build_path_traversal_parser(sub: argparse._SubParsersAction) -> None:  # type: ignore[type-arg]
    """Purpose: Register 'path-traversal' under talos attack."""
    pt_p = sub.add_parser(
        "path-traversal",
        aliases=["lfi", "path_traversal"],
        help="Path traversal / LFI testing.",
        description=(
            "Scan operator-picked flows for path traversal and local file "
            "inclusion.\n\n"
            "Pass --flow UUID (repeatable). The engine walks query parameters,\n"
            "JSON body fields/indexes, form fields, multipart filenames, and\n"
            "path parameters, then replaces the value with Unix, Windows,\n"
            "encoded, wrapper, null-byte, and bypass payloads. Optional\n"
            ...
            "created when a probe leaks a well-known file that was not in the\n"
            "captured baseline."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    ssub = pt_p.add_subparsers(dest="path_traversal_cmd", metavar="<command>")
    ssub.required = True

    tech_p = ssub.add_parser("techniques", help="List path-traversal payload techniques.")
    tech_p.add_argument(
        "--family",
        metavar="NAME",
        choices=list(FAMILIES),
        help="Restrict to unix | windows | dotdot | encoded | wrapper | nullbyte | bypass.",
    )
    add_format_argument(tech_p)

    run_p = ssub.add_parser(
        "run",
        help="Enqueue path-traversal probe jobs for one or more flows.",
        description=(
            "Scan captured flows for path traversal / LFI.\n\n"
            "Examples:\n"
    ...
    run_p.add_argument(
        ...
        default=True,
        help=(
            f"Enqueue at priority {PRIORITY_HIGH} so path-traversal runs before "
    ...
    res_p = ssub.add_parser("results", help="Inspect path-traversal probe results.")
    rsub = res_p.add_subparsers(dest="path_traversal_results_cmd", metavar="<command>")
    rsub.required = True
    rlist = rsub.add_parser("list", help="List path-traversal results.")
    rlist.add_argument("--verdict", choices=["PATH_TRAVERSAL", "SECURE", "UNKNOWN"])
...
def run_path_traversal_cli(manager: ProjectManager, args: argparse.Namespace) -> None:
    """Purpose: Dispatch path-traversal subcommands."""
    cmd = args.path_traversal_cmd
    if cmd == "techniques":
        cmd_techniques(manager, args)
    elif cmd == "run":
        cmd_run(manager, args)
    elif cmd == "results":
        if args.path_traversal_results_cmd == "list":
            cmd_results_list(manager, args)
        elif args.path_traversal_results_cmd == "show":
            cmd_results_show(manager, args)
        else:
            cli_usage_error("usage: talos attack path-traversal results list|show")
    elif cmd == "status":
        cmd_status(manager, args)
    else:
        cli_usage_error(f"unknown path-traversal command: {cmd}")
