"""
Module: talos.path_traversal.findings_bridge

Purpose:
    Create TRIAGING findings when a path-traversal job settles with
    PATH_TRAVERSAL.

    Clustering: PATH_TRAVERSAL:<endpoint_id> so every successful technique
    on the same endpoint lands under one PRIMARY (later techniques LINKED).

Dependencies: findings.creator / model / db
Data flow: scheduler settle / CLI --right-now → maybe_create_path_traversal_finding
Side effects: findings + evidence + timeline on PATH_TRAVERSAL.
"""
...
def build_finding_title(
    *,
    ...
    """
    label = ATTACK_DISPLAY.get(ATTACK_MODULE, "Path Traversal / LFI")
...
def maybe_create_path_traversal_finding(
    *,
    db_path: Path,
    project_id: str,
    ...
    """
    Purpose:
        Create a finding when the probe confirmed a file leak.
    Output:
        Finding UUID or None (non-trigger / error).
    """
    if outcome.verdict != VERDICT_PATH_TRAVERSAL:
        return None
    if not outcome.replayed_flow_id:
        _log.debug("[path_traversal] skip finding: PATH_TRAVERSAL without replay_flow_id")
        return None
    ...
    try:
        finding_id = create_finding_from_verdict(
            ...
            job_id=job_id,
            attack_type="path_traversal_attack",
