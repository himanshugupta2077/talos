"""
Module: talos.path_traversal.candidates
...
Purpose:
    ...
    always names the flow(s). IV ``path_traversal`` candidates are a
    separate prioritization hint.

Dependencies: sqlite3, talos.projects.db, talos.projects.outscope,
              talos.proxy.scope, talos.path_traversal.inject
Data flow: CLI run → select_path_traversal_candidates_for_flows
...
def _candidate_from_row(row: sqlite3.Row) -> PathTraversalCandidate:
...
def select_path_traversal_candidates_for_flows(
    db_path: Path,
    *,
    in_scope_prefixes: list[str],
    flow_ids: list[str],
) -> tuple[list[PathTraversalCandidate], list[str]]:
    """
    Purpose:
