...
Purpose:
    Persist and query xss_results — one row per unique replay flow.

Dependencies: sqlite3, pathlib, talos.projects.db
Data flow: engine insert → CLI / Control Panel list/show
Side effects: writes xss_results; migrate_project_db on entry.
"""
...
def _now_iso() -> str:
    """Purpose: UTC timestamp for xss_results.created_at."""
...
def insert_xss_result(db_path: Path, row: dict) -> None:
    """
    Purpose:
        Store one XSS / HTMLI probe result keyed by the unique replay flow.
    Side effects: INSERT into xss_results.
    """
    ...
    with _connect_rw(db_path) as conn:
        conn.execute(
            ...
            INSERT INTO xss_results (
                ...
                context_hint, encoding_hint, evidence, verdict, risk_hint,
...
def list_xss_results(
    ...
    *,
    ...
    """
    Purpose:
        List XSS probe results joined to the unique replay flow.
    Output:
...
def get_xss_result(db_path: Path, replay_flow_id: str) -> Optional[dict]:
    """
    Purpose:
        Fetch one xss_results row by unique replay flow UUID.
    Output:
        Dict or None.
    """
    migrate_project_db(db_path)
...
def count_xss_verdicts(db_path: Path) -> dict[str, int]:
    """
    Purpose:
    ...
    with _connect_rw(db_path) as conn:
        rows = conn.execute(
            "SELECT verdict, COUNT(*) AS n FROM xss_results GROUP BY verdict"
        ).fetchall()
