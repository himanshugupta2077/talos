...
Purpose:
    ...
    always names the flow(s). IV ``xss`` candidates are a separate
    prioritization hint.

Dependencies: sqlite3, talos.projects.db, talos.projects.outscope,
              talos.proxy.scope, talos.xss.inject
Data flow: CLI run → select_xss_candidates_for_flows
...
def _candidate_from_row(row: sqlite3.Row) -> XssCandidate:
...
def select_xss_candidates_for_flows(
    db_path: Path,
    *,
    in_scope_prefixes: list[str],
    flow_ids: list[str],
) -> tuple[list[XssCandidate], list[str]]:
    """
    Purpose:
