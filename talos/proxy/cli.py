...
def cmd_config(manager: ProjectManager, args: argparse.Namespace) -> None:
    """
    Purpose:
        Show or update the bound project's proxy startup mode.
        Config commits notify ProxyRuntimeManager (auto-restart when enabled).
    """
    project = _require_project(manager)
    ...
    if args.upstream:
        ...
        notify_proxy_config_changed(project.id, f"proxy config upstream {stored}")
        return
    ...
    if args.no_upstream:
        ...
        notify_proxy_config_changed(project.id, "proxy config no-upstream")
        return
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_config = sub.add_parser(
        "config",
        help="Show or update the proxy startup mode (Direct vs Upstream Proxy).",
    )
