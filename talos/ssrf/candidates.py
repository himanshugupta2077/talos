...
Purpose:
    ...
    always names the flow(s). IV ``ssrf`` candidates are a
    separate prioritization hint.

Dependencies: sqlite3, talos.projects.db, talos.projects.outscope,
              talos.proxy.scope, talos.ssrf.inject
Data flow: CLI run → select_ssrf_candidates_for_flows
...
def select_ssrf_candidates_for_flows(
    db_path: Path,
    *,
