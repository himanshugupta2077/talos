...
Dependencies: httpx, time, talos.replay.db, talos.projects.annotations,
              talos.ssrf.db / detect / inject / models
Data flow: scheduler / CLI --right-now → execute_ssrf_job → unique flow
Side effects: outbound HTTP; INSERT flows + ssrf_results.
"""
...
from talos.ssrf.db import insert_ssrf_result
from talos.ssrf.detect import analyze_ssrf_response
from talos.ssrf.inject import InjectionPoint, apply_payload
...
async def execute_ssrf_job(
    ...
    """
    Purpose:
        Run one ssrf payload against one entry point; store a unique flow.
    """
    technique = str(meta.get("technique") or "lb_http_127")
    ...
    oast_host = str(meta.get("oast_host") or "")
    oast_token = str(meta.get("oast_token") or "")
    try:
    ...
    replayed: dict = {
        ...
        "original_flow_id": flow_id,
        "replay_error": None,
        "replay_reason": "ssrf_attack",
    }

    from talos.burp.outbound import prepare_send_headers
    from talos.burp.trace import ENGINE_SSRF
    ...
    try:
        ...
        _log.error(
            "Failed to store ssrf replay flow %s: %s",
            replayed_flow_id,
            exc,
        )
        return _fail(
    ...
    if failure_reason:
        ...
        verdict, risk_hint, sink_hint, evidence = analyze_ssrf_response(
    ...
    try:
        ...
        _log.warning(
            "Failed to store ssrf_results for %s: %s",
            replayed_flow_id,
            exc,
        )
