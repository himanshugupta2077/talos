"""
Module: talos.ssrf.db

Purpose:
    Persist and query ssrf_results — one row per unique replay flow.

Dependencies: sqlite3, pathlib, talos.projects.db
Data flow: engine insert → CLI / Control Panel list/show
Side effects: writes ssrf_results; migrate_project_db on entry.
"""
...
def _now_iso() -> str:
    """Purpose: UTC timestamp for ssrf_results.created_at."""
...
def insert_ssrf_result(db_path: Path, row: dict) -> None:
    """
    Purpose:
        Store one SSRF probe result keyed by the unique replay flow.
    Side effects: INSERT into ssrf_results.
    """
    migrate_project_db(db_path)
    with _connect_rw(db_path) as conn:
        conn.execute(
            """
            INSERT INTO ssrf_results (
                ...
                sink_hint, oast_host, evidence, verdict, risk_hint,
...
def list_ssrf_results(
    ...
    *,
    ...
    """
    Purpose:
        List SSRF probe results joined to the unique replay flow.
    Output:
...
def get_ssrf_result(db_path: Path, replay_flow_id: str) -> Optional[dict]:
    """
    Purpose:
        Fetch one ssrf_results row by unique replay flow UUID.
    Output:
        Dict or None.
    """
    migrate_project_db(db_path)
...
def count_ssrf_verdicts(db_path: Path) -> dict[str, int]:
    """
    Purpose:
    ...
    with _connect_rw(db_path) as conn:
        rows = conn.execute(
            "SELECT verdict, COUNT(*) AS n FROM ssrf_results GROUP BY verdict"
        ).fetchall()
