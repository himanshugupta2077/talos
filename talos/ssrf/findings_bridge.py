"""
Module: talos.ssrf.findings_bridge

Purpose:
    Create TRIAGING findings when an SSRF job settles with SSRF.

    Clustering: SSRF:<endpoint_id> so every successful technique on the
    same endpoint lands under one PRIMARY (later techniques LINKED).

Dependencies: findings.creator / model / db
Data flow: scheduler settle / CLI --right-now → maybe_create_ssrf_finding
Side effects: findings + evidence + timeline on SSRF.
"""
...
from talos.ssrf.models import ATTACK_MODULE, VERDICT_SSRF, SsrfOutcome
...
def build_finding_title(
    *,
    ...
    """
    label = ATTACK_DISPLAY.get(ATTACK_MODULE, "SSRF")
...
def maybe_create_ssrf_finding(
    *,
    ...
    """
    Purpose:
        Create a finding when the probe confirmed in-band SSRF.
    Output:
        Finding UUID or None (non-trigger / error).
    """
    if outcome.verdict != VERDICT_SSRF:
        return None
    if not outcome.replayed_flow_id:
        _log.debug("[ssrf] skip finding: SSRF without replay_flow_id")
        return None
