...
class Project:
    ...
    Fields:
        ...
        scope       — List of Basic Scope URL/host prefixes defining capture
                      allow rules (Burp-style normal scope). One complete
                      prefix per entry. Examples: "example.com",
                      "http://example.com:8000", "https://app.example.com/api/".
                      Subdomains are not implied; wildcards are not accepted.
    ...
    def headers_drop_path(self) -> Path:
        """
        Path to the legacy per-project header filter file.
        Prefer capture.drop_headers in project.yaml / global config.yaml
        (CLI-022). This file remains as a legacy bridge when present.
    ...
    def project_config_path(self) -> Path:
        """
        Path to the layered project configuration overrides file (CLI-022).
        Only stores values that differ from global / defaults.
        """
        return Path(self.data_dir) / "project.yaml"
    ...
    def auth_session_path(self, role_id: str) -> Path:
        """
        Path to the persistent manual-session config file for one role.
        Created on demand via: talos auth-config set-session <role> path
        Edited by hand (any text editor) and then loaded via:
            talos auth-config set-session <role>
        Lives inside the project's data directory so it survives independently
        of any single editor/OS and is never a throwaway temp file.
        """
        return Path(self.data_dir) / "auth_sessions" / f"{role_id}.txt"
