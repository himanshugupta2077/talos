...
Purpose:
    Convert raw captured request paths and query strings into stable path/query
    forms used with application-origin identity. Endpoint identity is:

        HTTP method + canonical origin + normalized_path

    where canonical origin comes from talos.url_identity (scheme + authority,
    default ports elided). The cleaned query is retained for flow storage and
    parameter analysis but does not participate in endpoint deduplication.
