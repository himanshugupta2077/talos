"""
Module: talos.projects.bac.reclassify

Purpose:
    Offline re-application of BAC-decision-filter.yaml against stored
    bac_results. Used after an analyst updates the decision filter so that
    historical POSSIBLE_BAC noise can be reclassified to SECURE and linked
    findings auto-rejected as false positives.

Pipeline:
    load filter
        → for each bac_results row
            → load replay flow response
            → evaluate_response
            → update bac_results when verdict/match metadata changes
            → if POSSIBLE_BAC → SECURE: reject eligible findings + timeline reason
            → if non-POSSIBLE_BAC → POSSIBLE_BAC: count would_create_finding (v1 report only)

Status policy:
    TRIAGING  → REJECT on POSSIBLE_BAC→SECURE
...
Data flow:
    filter_cli / control-panel API → apply_bac_decision_filter → DB writes
Side effects:
    Updates bac_results and findings when dry_run=False.
"""
...
@dataclass
class ApplySummary:
    """Aggregate result of apply_bac_decision_filter."""
    dry_run: bool
    ...
    *,
    ...
    if decision_filter is None:
        ...
        raise FilterApplyError(
            ...
            "Run 'talos attack bac filter init' and edit the file, "
            "or fix YAML syntax, then re-run apply."
        )

    results = replay_db.list_bac_results(db_path)
...
def _process_one_result(
    *,
    ...
    resp = build_response_data(_normalize_flow_for_filter(flow))
    new_decision: DecisionResult = evaluate_response(decision_filter, resp)
    ...
    findings = findings_db.find_findings_by_evidence_ref(
        db_path, EVIDENCE_TYPE_BAC_RESULT, replay_id
    )
    ...
    # Reverse: non-POSSIBLE_BAC → POSSIBLE_BAC (v1 report only).
    if old_verdict != VERDICT_POSSIBLE_BAC and new_verdict == VERDICT_POSSIBLE_BAC:
        if not meta_same and not dry_run:
            replay_db.update_bac_result_verdict(
                db_path,
                replay_id,
                verdict=new_verdict,
        ...
        # Finding already exists for a non-POSSIBLE_BAC→POSSIBLE_BAC flip (unusual);
        # treat as result update.
    ...
    # Auto-reject findings only on POSSIBLE_BAC → SECURE (pass / false-positive path).
    if (
        old_verdict == VERDICT_POSSIBLE_BAC
        and new_verdict == VERDICT_SECURE
        and finding
    ):
...
def _normalize_flow_for_filter(flow: dict) -> dict:
    """
    Ensure response_body is bytes (or None) so build_response_data can decode.
    SQLite / helpers may surface body as str or buffer.
    """
    out = dict(flow)
    ...
    if isinstance(body, str):
        out["response_body"] = body.encode("utf-8", errors="replace")
        return out
    out["response_body"] = bytes(body)
    return out
...
def _format_match_reason(decision: DecisionResult, replay_id: str) -> str:
    ...
    return (
        ...
        f"(reclassified bac result {short})"
    )
