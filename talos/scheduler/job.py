...
"""Parser Confusion: duplicate params, HTTP parameter pollution, duplicate headers,
duplicate Content-Type, conflicting Content-Length/Transfer-Encoding."""
...
# ------------------------------------------------------------------ #
# Unauth attack job type constant                                      #
# ------------------------------------------------------------------ #

UNAUTH_ATTACK = "unauth_attack"
"""Unauthenticated access attack: auth mutation × optional request mutation.
Each job encodes one (auth_mutation, request_mutation) composition in its meta."""
...
IV_MULTIPROBE = "iv_multiprobe"
"""Phase 1b — Single multi-signal probe (canary + taxonomy samples). Module 4."""

IV_IDENTIFIER = "iv_identifier"
"""Phase 2 — Inject traceable canary identifiers to detect reflection."""

IV_CHARACTERS = "iv_characters"
"""Phase 3 — Character acceptance testing (per-char; reduced when multiprobe on)."""
...
"""Job is suspended due to session expiry.  Resumes automatically when the
tester provides new authentication artifacts and runs 'talos scheduler resume'."""

STATUS_CANCELLED = "cancelled"
"""Job was permanently cancelled by the tester."""
