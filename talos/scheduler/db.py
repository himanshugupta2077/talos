...
Purpose:
    ...
    and the CLI management commands:
        ...
        list_jobs              — inventory with optional status/type filters.
        get_job                — resolve one job by full UUID or unique prefix.
        cancel_job             — mark a single pending/paused job cancelled.
        clear_pending_jobs     — bulk-delete pending jobs.
        prune_jobs             — delete terminal-status history (done/failed/…).
...
Data flow:
    scheduler.cli → enqueue_job() / list_jobs() / get_job() / cancel_job() /
                    prune_jobs() → DB
...
# Terminal statuses eligible for history prune (not active queue).
PRUNEABLE_STATUSES: frozenset[str] = frozenset(
    {STATUS_DONE, STATUS_FAILED, STATUS_SKIPPED, STATUS_CANCELLED}
)

# Statuses a tester may cancel without racing mid-execution.
CANCELLABLE_STATUSES: frozenset[str] = frozenset({STATUS_PENDING, STATUS_PAUSED})

# Columns selected when hydrating ReplayJob rows.
_JOB_SELECT_COLS = (
...
# Scheduler execution state constants.
SCHED_STATE_RUNNING = "running"
SCHED_STATE_PAUSED = "paused"
SCHED_STATE_WAITING_FOR_SESSION = "waiting_for_session"
...
def mark_paused(db_path: Path, job_id: str) -> None:
    """
    Purpose:
        Transition a job to 'paused' state when the scheduler suspends execution
        due to session expiry.  Unlike failed/skipped, paused jobs are resumable
        once new authentication artifacts are provided.
    Input:
        db_path — Path to the project's talos.db.
        job_id  — UUID of the job to pause.
    Side effects:
        Updates one row in scheduler_jobs; commits.
    """
    with _connect_rw(db_path) as conn:
        conn.execute(
            "UPDATE scheduler_jobs SET status = ? WHERE job_id = ?",
            (STATUS_PAUSED, job_id),
        )
        conn.commit()
...
def pause_pending_jobs(db_path: Path) -> int:
    """
    Purpose:
        Transition all pending jobs to 'paused' state.  Called when the scheduler
        detects a MANUAL session expiry to prevent queued jobs from failing.
    Input:   db_path — Path to the project's talos.db.
    Output:  Number of jobs paused.
    Side effects:
        Updates scheduler_jobs rows; commits.
    """
    migrate_project_db(db_path)
    with _connect_rw(db_path) as conn:
        cursor = conn.execute(
            "UPDATE scheduler_jobs SET status = ? WHERE status = ?",
...
def resume_paused_jobs(db_path: Path) -> int:
    """
    Purpose:
        Transition all paused jobs back to 'pending' state.  Called by
        'talos scheduler resume' after the tester has updated their session.
    Input:   db_path — Path to the project's talos.db.
    Output:  Number of jobs returned to pending.
    Side effects:
        Updates scheduler_jobs rows; commits.
    """
    migrate_project_db(db_path)
...
def get_scheduler_state(db_path: Path) -> str:
    """
    Purpose:
        Return the current global scheduler execution state.
    Input:   db_path — Path to the project's talos.db.
    Output:  One of SCHED_STATE_RUNNING, SCHED_STATE_PAUSED,
             SCHED_STATE_WAITING_FOR_SESSION.
             Returns SCHED_STATE_RUNNING when no row is stored.
    Side effects: None (read-only).
    """
    migrate_project_db(db_path)
    with _connect_rw(db_path) as conn:
        row = conn.execute(
            "SELECT state FROM scheduler_state WHERE id = 'default'"
        ).fetchone()
    return row[0] if row else SCHED_STATE_RUNNING
...
def set_scheduler_state(
    ...
    """
    Purpose:
        Write the global scheduler execution state to the DB.
        The scheduler loop reads this on every iteration.
    Input:
        db_path — Path to the project's talos.db.
        state   — SCHED_STATE_RUNNING | SCHED_STATE_PAUSED |
                  SCHED_STATE_WAITING_FOR_SESSION.
        reason  — Optional human-readable explanation.
    Output: None
    Side effects:
        Upserts one row in scheduler_state; commits.
    """
    migrate_project_db(db_path)
    ...
    with _connect_rw(db_path) as conn:
        conn.execute(
            """
            INSERT INTO scheduler_state (id, state, reason, updated_at)
            VALUES ('default', ?, ?, ?)
...
def list_jobs_by_status(
    ...
    Purpose:
        ...
        for terminal statuses (done/failed/skipped/cancelled) or created_at
        DESC otherwise. Used by status and reporting callers that need
...
def list_jobs(
    ...
    *,
    ...
    """
    Purpose:
        Return a filtered job inventory for 'talos scheduler jobs list'.
        Ordering: finished_at DESC for terminal-only status filters; otherwise
        created_at DESC (newest first) for mixed inventory browsing.
    Input:
        db_path    — Path to the project's talos.db.
        project_id — Project identifier for ReplayJob construction.
        status     — Optional exact status filter.
        job_type   — Optional type filter. Exact match, or prefix family when
                     the value has no underscore (e.g. 'replay' matches
                     replay_flow and replay_endpoint; 'bac' matches bac_*).
    ...
    if job_type is not None:
        # Family prefix: "replay" → replay_% ; exact token otherwise.
        if "_" not in job_type:
            clauses.append("(job_type = ? OR job_type LIKE ?)")
            params.append(job_type)
            params.append(f"{job_type}_%")
        else:
    ...
    if status in PRUNEABLE_STATUSES:
        order_col = "finished_at DESC, created_at DESC"
    else:
...
def get_job(
    ...
    """
    Purpose:
        Resolve a single job by full UUID or unique id prefix for show/cancel.
    Input:
        db_path    — Path to the project's talos.db.
        project_id — Project identifier for ReplayJob construction.
        job_id     — Full job UUID or a unique prefix (case-insensitive).
    Output:
        ReplayJob when exactly one row matches; None when no match.
    ...
    with _connect_rw(db_path) as conn:
        # Prefer exact match first (full UUID).
        row = conn.execute(
            f"""
            SELECT {_JOB_SELECT_COLS}
            FROM scheduler_jobs
    ...
    if not rows:
        return None
    if len(rows) > 1:
        matches = ", ".join(r["job_id"][:12] + "…" for r in rows)
        raise ValueError(
            f"Ambiguous job id prefix '{needle}' matches multiple jobs: {matches}. "
            "Provide a longer prefix or the full UUID."
        )
    return _row_to_job(rows[0], db_path, project_id)


def cancel_job(db_path: Path, job_id: str) -> Optional[str]:
    """
    Purpose:
        Permanently cancel one pending or paused job (STATUS_CANCELLED).
        Running jobs are not cancelled here (they are mid-execution); terminal
        jobs are already finished.
    Input:
        db_path — Path to the project's talos.db.
        job_id  — Full job UUID (caller resolves prefixes via get_job).
    Output:
        Previous status string when the job was cancelled; None when the job
        does not exist or is not in a cancellable state.
    Side effects:
        May update one row in scheduler_jobs; commits.
    """
    migrate_project_db(db_path)
    now = _now_iso()
    ...
    with _connect_rw(db_path) as conn:
        row = conn.execute(
            "SELECT status FROM scheduler_jobs WHERE job_id = ?",
            (job_id,),
        ).fetchone()
        if row is None:
            return None
...
def count_jobs_by_status(db_path: Path, status: str) -> int:
    """
    Purpose:
        Count jobs in a single status (used by prune confirmation).
    Input:
        db_path — Path to the project's talos.db.
    ...
    with _connect_rw(db_path) as conn:
        row = conn.execute(
            "SELECT COUNT(*) AS n FROM scheduler_jobs WHERE status = ?",
            (status,),
        ).fetchone()
...
def clear_pending_jobs(db_path: Path) -> int:
    ...
    Purpose:
        ...
        Running, done, failed, skipped, and cancelled jobs are not affected.
    Input:   db_path — Path to the project's talos.db.
...
def prune_jobs(db_path: Path, status: str) -> int:
    """
    Purpose:
        Delete terminal-status job history for one status bucket.
        Only done / failed / skipped / cancelled may be pruned; active queue
        statuses (pending / running / paused) must use clear or cancel instead.
    Input:
        db_path — Path to the project's talos.db.
        status  — One of PRUNEABLE_STATUSES.
    Output:
        Number of rows deleted.
    Side effects:
        Deletes matching rows from scheduler_jobs; commits.
    Raises:
        ValueError if status is not pruneable.
    """
    if status not in PRUNEABLE_STATUSES:
        allowed = ", ".join(sorted(PRUNEABLE_STATUSES))
        raise ValueError(
            f"Cannot prune status '{status}'. Allowed: {allowed}."
        )
...
def get_scheduler_config(db_path: Path) -> dict:
    """
    Purpose:
        Read effective scheduler configuration via the layered configuration
        system (defaults → global → legacy SQLite / project.yaml).
...
def set_scheduler_config(
    ...
    Purpose:
        ...
        Also dual-writes scheduler.* into project.yaml (CLI-022).
    Input:
        db_path        — Path to the project's talos.db.
    ...
    try:
        ...
        current = set_path(current, "scheduler.min_delay", float(min_delay))
        current = set_path(current, "scheduler.max_delay", float(max_delay))
        current = set_path(current, "scheduler.max_queue_size", int(max_queue_size))
