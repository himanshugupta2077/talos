...
Dependencies: sqlite3, talos.projects.db, talos.projects.outscope,
              talos.proxy.scope, talos.sqli.inject
Data flow: CLI run → select_sqli_candidates_for_flows
...
def _candidate_from_row(row: sqlite3.Row) -> SqliCandidate:
    """Purpose: Build a SqliCandidate and extract entry points."""
    url = row["url"] or ""
...
def select_sqli_candidates_for_flows(
    db_path: Path,
    *,
    in_scope_prefixes: list[str],
    flow_ids: list[str],
) -> tuple[list[SqliCandidate], list[str]]:
    """
    Purpose:
