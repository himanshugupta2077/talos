"""
Module: talos.host_header.findings_bridge

Purpose:
    Create TRIAGING findings when a host-header job settles with
    HOST_HEADER.

    Clustering: HOST_HEADER:<endpoint_id> so every successful technique
    on the same endpoint lands under one PRIMARY (later techniques LINKED).

Dependencies: findings.creator / model / db
Data flow: scheduler settle / CLI --right-now → maybe_create_host_header_finding
Side effects: findings + evidence + timeline on HOST_HEADER.
"""
...
def build_finding_title(
    *,
    ...
    """
    Purpose:
        PRIMARY uses an endpoint-level title; LINKED names the header + technique.
    """
    label = ATTACK_DISPLAY.get(ATTACK_MODULE, "Host Header Injection")
    m = (method or "?").strip() or "?"
    p = (path or "?").strip() or "?"
    if is_primary:
        return f"{label} — {m} {p}"
    point = param_name or "Host"
    return f"{label} — {technique} on {point} ({m} {p})"


def maybe_create_host_header_finding(
    *,
    db_path: Path,
    project_id: str,
    ...
    """
    Purpose:
        Create a finding when the probe confirmed host-header injection.
    Output:
        Finding UUID or None (non-trigger / error).
    """
    if outcome.verdict != VERDICT_HOST_HEADER:
        return None
    if not outcome.replayed_flow_id:
        _log.debug("[host_header] skip finding: HOST_HEADER without replay_flow_id")
        return None
    ...
    try:
        finding_id = create_finding_from_verdict(
            ...
            job_id=job_id,
            attack_type="host_header_attack",
