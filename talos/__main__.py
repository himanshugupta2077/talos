...
def _print_usage() -> None:
    print(
        ...
        "    auth-mode       Show or set artifacts | platform_ntlm\n"
        ...
        "    config          Show or set Direct vs Upstream, HTTP/1.1, keep-alive\n"
        "                    [--http1] [--keep-alive] (also: talos config proxy)\n"
        "    auth            Platform authentication (NTLMv2) toward origin hosts\n"
        ...
        "    bind-ntlm       Bind a role to an NTLM / platform-auth profile\n"
        "    unbind-ntlm     Remove the NTLM profile binding for a role\n"
        "    show-ntlm       Show NTLM profile binding(s)\n\n"
        "  endpoint\n"
        "    list            List endpoints (UUID, method, host, path, policy)\n"
        "                    [--method] [--host] [--qualified] [--excluded]\n"
        "                    [--search] [--role] [--priority] [--format table|json]\n"
        "                    Parameter inventory (path/query/body/header/cookie/\n"
        "                    response) + url_features is built automatically by\n"
        "                    FlowWorker (URL Sink Discovery Phases 1–2; schema v53).\n"
        "    params <id>     List parameter inventory + url_features for an endpoint\n"
        "                    [--min-score N] [--network-resource] [--location LOC]\n"
        "                    [--format table|json]\n"
        ...
        "  send              Repeater: edit → send → review (mutable lineage)\n"
        "    from <flow_id>  Materialize raw HTTP draft (no send, no DB write)\n"
        "                    [--raw-out PATH] [--format table|json]\n"
        "    edit <flow_id>  Open draft in $EDITOR; optional --send after save\n"
        "                    [--editor CMD] [--send] [--raw-out PATH]\n"
        "    once <flow_id>  Apply edits, send, store new flow(s) + lineage\n"
        "                    [--method] [--url] [--path] [--host] [--header]\n"
        "                    [--remove-header] [--cookie] [--remove-cookie]\n"
        "                    [--query] [--remove-query] [--json-set KEY=VALUE]\n"
        "                    [--body|--body-file|--body-stdin] [--raw-file]\n"
        "                    [--no-update-content-length] [--no-sync-host]\n"
        "                    [--session UUID] [--note TEXT] [--reason TEXT]\n"
        "                    [--source manual_send|ai_send]\n"
        "                    [--repeat N|--parallel N] [--delay-ms M]\n"
        "                    [--format table|json]\n"
        "    redo <flow_id>  Re-send exact as-sent request (new child row)\n"
        "    dup <flow_id>   New session_id branch marker (no HTTP)\n"
        "    show <flow_id>  Request/response [--body request|response|both]\n"
        "                    [--full] [--format table|json]\n"
        "    export <id>     Write request.http + response under --out DIR\n"
        "    history         List sends under a baseline/root\n"
        "                    --from <flow_id> [--session] [--parent] [--source]\n"
        ...
        "  intruder          High-volume mutation engine (Phase 1–5 CLI)\n"
        ...
        "                    clone copies config to a new draft (no results)\n"
        "    template show|set-var|clear-var|from-params\n"
        "                    {{variables}} on baseline; from-params pulls Parameter Intelligence\n"
        "    payload set|list|clear\n"
        "                    Generators: wordlist, numbers, static, uuid, csv, json,\n"
        "                    example_values, pool, dates, bruteforce, random, pattern;\n"
        "                    processors: encode/decode/hash/prefix/suffix…\n"
        "    strategy set    single | sniper | pitchfork | zip | cluster_bomb\n"
        "    timing set      fixed | unlimited | token_bucket | adaptive;\n"
        "                    --rps, --concurrency, --concurrency-per-host,\n"
        "                    --min-rps/--max-rps/--slow-ms/--burst-size (Phase 4)\n"
        "    storage set|show  metrics_only | sample_flows | all_flows\n"
        "    match add|list|clear\n"
        "                    status, body_contains, regex, length_delta_gt, time_gt_ms\n"
        "    grep add|list|clear\n"
        "                    Extract regex captures → grepped_json + optional project pools\n"
        "    pool list|show|export|clear|delete\n"
        "                    Project-scoped extracted value pools (chain into next session)\n"
        "    findings set|show|promote\n"
        "                    Optional findings promote (default off; max_findings cap)\n"
        "    suggest         Offline AI/operator config suggestions [--apply]\n"
        "    results list|show|export\n"
        "                    Metrics + interesting tags + grepped; export JSONL/CSV\n"
        "    generators list Plugins inventory (generators/processors/strategies/timing)\n"
        ...
        "                    [--technique NAME] [--flow UUID]\n"
        "    unauth config   Show or set auto-run (auth_test auto-enqueue)\n"
        "                    [show] [--auto-run on|off]\n"
        "    unauth filter   init | show | validate | apply\n"
        "                    apply [--dry-run] [--force]\n"
        "    auth-session bind|unbind|show-bindings\n"
        "                    Bind auth_config field → jwt; auto-picks top 5 flows\n"
        "    auth-session generate\n"
        "                    Create mutation tests for selected flows\n"
        "                    [--flow|--endpoint|--module] [--force-refresh]\n"
        "                    [--include-unsafe-methods] [--test-id|--family]\n"
        "                    [--format table|json]\n"
        "    auth-session candidates list|show|add|remove\n"
        "                    Target flows (GET/POST/PATCH) [--flow] [--format]\n"
        "    auth-session approve|reject|unapprove\n"
        "                    Optional lifecycle (run no longer requires approve)\n"
        "                    [--all-pending|--retry-failed|--all-approved]\n"
        "                    [--test-id|--family|--endpoint] [--format]\n"
        "    auth-session run\n"
        "                    Enqueue JWT tests (latest or --jwt TOKEN)\n"
        "                    [--candidate|--endpoint|--test-id|--family]\n"
        "                    [--jwt TOKEN] [--right-now] [--format]\n"
        "    auth-session results list|show\n"
        "                    Inspect WEAK_VALIDATION / SECURE / UNKNOWN\n"
        "    auth-session status\n"
        "                    Bindings + candidate/result tallies [--format]\n"
        ...
        "                    [--module NAME|UUID] [--flow UUID]\n"
        "                    [--auto-generate]\n"
        "    bac filter      init | show | validate\n"
        "    cors candidates  List in-scope 200 OK baselines\n"
        "                    [--limit|--endpoint|--host|--flow] [--format]\n"
        "    cors techniques  Origin payload catalogue [--format]\n"
        "    cors run         Enqueue one cors_attack job per technique\n"
        "                    (unique replay flow each). [--technique]\n"
        "                    [--limit|--endpoint|--host|--flow] [--right-now]\n"
        "    cors results list|show\n"
        "                    Inspect CORS_MISCONFIG / SECURE / UNKNOWN\n"
        "    cors status      Verdict + job tallies [--format]\n"
        "    sqli techniques  SQL payload catalogue [--format]\n"
        "    sqli run         Enqueue one sqli_attack job per entry\n"
        "                    point × payload (unique replay flow each).\n"
        "                    --flow UUID required. [--family|--technique]\n"
        "                    [--right-now] [--high-priority|--no-high-priority]\n"
        "                    (default: high — SQLi runs before other pending jobs)\n"
        "    sqli results list|show\n"
        "                    Inspect SQLI / SECURE / UNKNOWN\n"
        "    sqli status      Verdict + job tallies [--format]\n"
        "    xss techniques   XSS / HTMLI catalogue [--family] [--format]\n"
        "    xss run          Enqueue one xss_attack job per\n"
        "                    entry point × payload (unique replay flow).\n"
        "                    --flow UUID required. [--param|--family|--technique]\n"
        "                    [--right-now] [--high-priority|--no-high-priority]\n"
        "    xss results list|show\n"
        "                    Inspect XSS / HTMLI / SECURE / UNKNOWN\n"
        "    xss status       Verdict + job tallies [--format]\n"
        "    path-traversal techniques\n"
        "                    LFI / traversal catalogue [--family] [--format]\n"
        "    path-traversal run\n"
        "                    Enqueue one path_traversal_attack job per\n"
        "                    entry point × payload (unique replay flow).\n"
        "                    --flow UUID required. [--param|--family|--technique]\n"
        "                    [--right-now] [--high-priority|--no-high-priority]\n"
        "    path-traversal results list|show\n"
        "                    Inspect PATH_TRAVERSAL / SECURE / UNKNOWN\n"
        "    path-traversal status\n"
        "                    Verdict + job tallies [--format]\n"
        "    ssrf techniques  SSRF payload catalogue [--family] [--format]\n"
        "    ssrf run         Enqueue one ssrf_attack job per\n"
        "                    entry point × payload (unique replay flow).\n"
        "                    --flow UUID required. [--param|--family|--technique]\n"
        "                    [--collaborator HOST_OR_URL]\n"
        "                    [--right-now] [--high-priority|--no-high-priority]\n"
        "    ssrf results list|show\n"
        "                    Inspect SSRF / SECURE / UNKNOWN\n"
        "    ssrf status      Verdict + job tallies [--format]\n"
        "    open-redirect techniques\n"
        "                    Open-redirect catalogue [--family] [--format]\n"
        "    open-redirect run\n"
        "                    Enqueue one open_redirect_attack job per\n"
        "                    entry point × payload (unique replay flow).\n"
        "                    --flow UUID required. [--param|--family|--technique]\n"
        "                    [--right-now] [--high-priority|--no-high-priority]\n"
        "    open-redirect results list|show\n"
        "                    Inspect OPEN_REDIRECT / SECURE / UNKNOWN\n"
        "    open-redirect status\n"
        "                    Verdict + job tallies [--format]\n"
        "    host-header techniques\n"
        "                    Host-header injection catalogue [--family] [--format]\n"
        "    host-header run\n"
        "                    Enqueue one host_header_attack job per\n"
        "                    header × payload (unique replay flow).\n"
        "                    --flow UUID required. [--header|--family|--technique]\n"
        "                    [--right-now] [--high-priority|--no-high-priority]\n"
        "    host-header results list|show\n"
        "                    Inspect HOST_HEADER / SECURE / UNKNOWN\n"
        "    host-header status\n"
        "                    Verdict + job tallies [--format]\n"
        "    smuggle techniques  CL/TE desync catalogue [--format]\n"
        "    smuggle run      Enqueue one smuggle_attack job per\n"
        "                    technique (unique replay flow each).\n"
        "                    --flow UUID required. [--technique]\n"
        "                    [--right-now]\n"
        "    smuggle results list|show\n"
        "                    Inspect SMUGGLE / SECURE / UNKNOWN\n"
        "    smuggle status   Verdict + job tallies [--format]\n\n"
        "  input-validation\n"
        "    run             Schedule IV jobs (adaptive planner)\n"
        "                    [--budget quick|standard|deep|exhaustive]\n"
        "                    [--host / --endpoint / --flow / --parameter NAME]\n"
        "                    [--ignore-cache] [--include-auth-artifacts]\n"
        "                    (types on + url_features warrant → url_sink_probes)\n"
        ...
        "    clear-cache     Reset probes/profiles/cache [--force]\n"
        "                    (optional host/endpoint/parameter)\n"
        ...
        "    show <uuid>                Show detail + original vs attack/testcase flows\n"
        ...
        "    config show|set            Secret detection settings\n"
        ...
        "    suggest         One planner turn (heuristic or configured LLM)\n"
        "                    [--auto-reads] [-n N]\n"
        "    approve <id>    Approve pending plan_id (or suggestion_id)\n"
        "    deny <id>       Deny plan or suggestion\n"
        "    pending         Suggestions + plans awaiting approval\n"
        "    plans show <id> ExecutionPlan vs immutable suggestion\n"
        "    notes           show|edit|export structured app notes\n"
        "    kb              list|show|search operator Markdown KB\n"
        "                    (~/.talos/ai/kb/*.md — add files manually)\n"
        "    finding         list-drafts|show-draft|promote|reject-draft\n"
        "                    (promote → TRIAGING finding; never confirm)\n"
        "    session export  JSON bundle (suggestions/plans/obs/audit)\n"
        "    tools list      Allowlisted ToolSpec inventory (no execute path)\n"
        "    mcp serve       stdio MCP over PolicyValidator+Executor\n"
        "                    [--session SESSION] (no network MCP)\n"
        "    config          show|set|unset|edit LLM provider settings\n"
        "                    (~/.talos/ai/config.yaml; never a tool)\n"
