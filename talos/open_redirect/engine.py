...
Dependencies: httpx, time, talos.replay.db, talos.projects.annotations,
              talos.open_redirect.db / detect / inject / models
Data flow: scheduler / CLI --right-now → execute_open_redirect_job → unique flow
Side effects: outbound HTTP; INSERT flows + open_redirect_results.
"""
...
from talos.open_redirect.db import insert_open_redirect_result
from talos.open_redirect.detect import analyze_open_redirect_response
from talos.open_redirect.inject import InjectionPoint, apply_payload
...
async def execute_open_redirect_job(
    flow_id: str,
    meta: dict,
    db_path: Path,
    project_id: str,
    ...
    """
    Purpose:
        Run one open-redirect payload against one entry point; store a unique flow.
    """
    technique = str(meta.get("technique") or "abs_https")
    ...
    canary_host = str(meta.get("canary_host") or "")
    try:
    ...
    if not param_name:
        return _fail(
            ...
            endpoint_id=endpoint_id,
            original_status=original_status,
            reason="open_redirect_point_missing",
        )
    ...
    replayed: dict = {
        ...
        "original_flow_id": flow_id,
        "replay_error": None,
        "replay_reason": "open_redirect_attack",
    }

    from talos.burp.outbound import prepare_send_headers
    from talos.burp.trace import ENGINE_OPEN_REDIRECT
    ...
    try:
        ...
        _log.error(
            "Failed to store open-redirect replay flow %s: %s",
            replayed_flow_id,
            exc,
        )
        return _fail(
    ...
    if failure_reason:
        ...
        from talos.open_redirect.models import CANARY_HOST

        verdict, risk_hint, redirect_url, evidence = analyze_open_redirect_response(
            ...
            canary_host=canary_host or CANARY_HOST,
            payload_sent=payload_sent,
        )
        outcome = OpenRedirectOutcome(
            original_flow_id=flow_id,
    ...
    try:
        ...
        _log.warning(
            "Failed to store open_redirect_results for %s: %s",
            replayed_flow_id,
            exc,
        )
