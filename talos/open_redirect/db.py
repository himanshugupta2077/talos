"""
Module: talos.open_redirect.db

Purpose:
    Persist and query open_redirect_results — one row per unique replay flow.

Dependencies: sqlite3, pathlib, talos.projects.db
Data flow: engine insert → CLI / Control Panel list/show
Side effects: writes open_redirect_results; migrate_project_db on entry.
"""
...
def _now_iso() -> str:
    """Purpose: UTC timestamp for open_redirect_results.created_at."""
...
def insert_open_redirect_result(db_path: Path, row: dict) -> None:
    """
    Purpose:
        Store one open-redirect probe result keyed by the unique replay flow.
    Side effects: INSERT into open_redirect_results.
    """
    migrate_project_db(db_path)
    with _connect_rw(db_path) as conn:
        conn.execute(
            """
            INSERT INTO open_redirect_results (
                ...
                redirect_url, evidence, verdict, risk_hint,
...
def list_open_redirect_results(
    db_path: Path,
    *,
    ...
    """
    Purpose:
        List open-redirect probe results joined to the unique replay flow.
    """
    migrate_project_db(db_path)
...
def get_open_redirect_result(db_path: Path, replay_flow_id: str) -> Optional[dict]:
    """Purpose: Fetch one open_redirect_results row by replay flow UUID."""
    migrate_project_db(db_path)
    if not db_path.exists():
        return None
    with _connect_rw(db_path) as conn:
...
def count_open_redirect_verdicts(db_path: Path) -> dict[str, int]:
    """Purpose: Verdict histogram for status / overview KPIs."""
    migrate_project_db(db_path)
    if not db_path.exists():
        return {}
    with _connect_rw(db_path) as conn:
        rows = conn.execute(
            "SELECT verdict, COUNT(*) AS n FROM open_redirect_results GROUP BY verdict"
        ).fetchall()
