    ...
    - role_id and module_id are resolved by the addon at proxy start and carried
      ...
      or defaults role/module; a flow missing either is dropped.
        - Every persisted flow is normalized into a stable endpoint identity using
            (method, canonical_origin, normalized_path). The `host` column stores
            the canonical origin so ports/schemes stay distinct.
...
class FlowWorker:
    ...
    def __init__(
        ...
        # Load out-of-scope prefixes once at worker startup as a safety backstop.
    ...
    def _process(self, flow: dict) -> None:
        ...
        # Out-of-scope safety backstop — shared Basic Scope matcher on the full
        # request URL (not hostname-only). Primary enforcement is the proxy addon
        # (full evaluate_scope). This catches races / direct queue injection.
        flow_url = (flow.get("url") or "").strip()
        if flow_url and any_rule_matches(flow_url, self._out_of_scope):
            logger.warning(
                "Dropping out-of-scope flow (worker backstop) — flow_id=%s url=%s",
                flow.get("flow_id", "?"),
                flow_url,
            )
            self.dropped_count += 1
...
def _persist_db(flow: dict, db_path: Path) -> Optional[str]:
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        # Qualification update: mark the endpoint as testable when this
        # proxy_capture flow returned a 2xx status.  Also caches the
        # baseline_flow_id so attack modules skip per-attack DB scans.
        # Runs after auto-priority so the policy row always exists.
        # Failure here is non-fatal — flow and endpoint are already committed.
        if endpoint_id is not None:
            try:
                update_endpoint_qualification(
                    conn=conn,
                    endpoint_id=endpoint_id,
                ...
                conn.rollback()
                logger.error(
                    "Qualification update failed — endpoint_id=%s flow_id=%s"
                    " — skipping, endpoint remains unqualified",
                    endpoint_id,
                    flow.get("flow_id"),
                    exc_info=True,
                )
...
def _endpoint_origin_key(flow: dict) -> str:
    """
    Purpose:
        Resolve the canonical origin used as endpoints.host identity.
        Prefer the capture-time host field (already origin from the addon);
        otherwise derive from the full URL.
    Side effects: None.
    """
    host = (flow.get("host") or "").strip()
    if host.startswith("http://") or host.startswith("https://"):
        return host
    url = (flow.get("url") or "").strip()
    if url:
        try:
            return parse_request_url(url).canonical_origin
...
def _upsert_endpoint(
    ...
    Purpose:
        ...
        Identity is method + canonical origin + normalized_path so that
        http://test.com:8000/api and http://test.com:9000/api stay distinct.
    Input:
    ...
    """
    origin = _endpoint_origin_key(flow)
    row = conn.execute(
        """
