...
Purpose:
    ...
    Both commands require a bound project (registry ACTIVE, --project, or TALOS_PROJECT). The project supplies
...
Dependencies: argparse, asyncio, sys, talos.projects.manager, talos.replay.engine,
              talos.replay.db
Data flow:
    CLI args → bound project DB → engine (async) → ReplayOutcome → stdout
Side effects:
    - Sends an outbound HTTP request.
    - Writes one replay flow to the project SQLite database.
    - Prints outcome to stdout.
    - Exits 3 if no project bound; exits 1 if flow/endpoint not found.
"""
...
def cmd_replay_flow(project: object, args: argparse.Namespace) -> None:
    ...
    if outcome.failure_reason == "endpoint_annotated_logout":
        cli_precondition_error(f"Flow '{flow_id}' belongs to a logout endpoint — replay blocked.")
...
def cmd_replay_endpoint(project: object, args: argparse.Namespace) -> None:
    ...
    if outcome.failure_reason == "no_qualifying_flow":
        cli_error(
            f"Endpoint '{endpoint_id}' has no 200 OK proxy_capture flow "
            "to replay."
        )

    if outcome.failure_reason == "endpoint_annotated_logout":
        cli_precondition_error(f"Endpoint '{endpoint_id}' is tagged logout — replay blocked.")

    if outcome.failure_reason == "endpoint_annotated_dangerous":
        cli_precondition_error(
            f"Endpoint '{endpoint_id}' is tagged dangerous — "
